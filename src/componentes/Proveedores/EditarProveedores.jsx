...
export function EditarProveedor() {
  ...
  const editar_proveedor = () => {
    ...
    if (isNaN(Number(cuil)) || isNaN(Number(id_productos))) {
      swal.fire({
        icon: "error",
        title: "ingresar solo valores numericos",
        showConfirmButton: false,
        timer: 3000,
      });
      return;
    ...
    swal.fire({
      icon: "success",
      title: "se edito el proveedor correctamente",
      showConfirmButton: false,
      timer: 2500,
    });
    setTimeout(() => {
      ...
      <div className="card-body">
        ...
        <div className="form-group">
          ...
          <input
            type="text"
            value={id_productos}
            onChange={(event) => setId_Productos(event.target.value)}
            name=""
            id=""
            className="form-control"
            placeholder=""
